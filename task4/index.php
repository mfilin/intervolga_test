...
// необходимо составить хитрую регулярку


$words_to_keep = 29;
$word_count = str_word_count(strip_tags($text), 0);

if ($word_count > $words_to_keep) {
    $words = preg_split('/\s+/', $text, $words_to_keep + 1);
    $trimmed_text = implode(' ', array_slice($words, 0, $words_to_keep)) . '...';
} else {
    $trimmed_text = $text;
}
